...
class Product:
    VOLCANIC_ROCK = "VOLCANIC_ROCK"# 去掉趋势判断
    VOUCHERS = [
    ...
    RAINFOREST_RESIN = "RAINFOREST_RESIN"# 管理得好啊
    KELP = "KELP"# 跟resin一样波动不大，因此用的resin的吃单做单平仓方法
    SQUID_INK = "SQUID_INK"# 根据PDF调整阈值，根据趋势图调整最大最小值。去掉趋势判断
    PICNIC_BASKET1 = "PICNIC_BASKET1"# 仔细看了一下，确实是条件设置错了导致篮子没有交易。对不起，然后改了改阈值
    CROISSANTS = "CROISSANTS"
    JAMS = "JAMS"
    DJEMBES = "DJEMBES"

    PICNIC_BASKET2 = "PICNIC_BASKET2"# 这个之后再看看怎么个事，先看期权和round4.
...
Product.MAGNIFICENT_MACARONS:{
    ...
    Product.KELP: {
        "fair_value": 2012,  # 设定商品的基准价格
        "take_width": 1,  # 允许吃单的价差阈值,当对手方订单价格与fair_value的偏离超过clear_width时吃单
        "clear_width": 0,  # 平仓订单的挂单价差范围,要平仓时在fair_value ± clear_width挂单。
        # for making
        "disregard_edge": 1,  # 忽略对手方订单的范围阈值，挂单时忽略对手方订单中与fair_value 过近的订单
        "join_edge": 4,  # 加入已有订单的价差容忍范围，当订单价格在fair_value ± join_edge内时挂相同价格的订单，否则挂更优价格。
        "default_edge": 5,  # 默认挂单价差,当没有可直接交易的订单时，以fair_value ± default_edge挂单。
...
class Trader:
    def __init__(self, max_position_change=50,params = None):  # 设置最大头寸变动值的默认参数
        ...
        self.price_history_makalong = []  # 价格历史数据
        ...
        self.KELP_WINDOW_SIZE = 20
        self.BUY_THRESHOLD = 0.4  # 买入阈值：低于最低价+极差的20%
        self.SELL_THRESHOLD = 0.6  # 卖出阈值：高于最低价+极差的80%
    ...
    def update_price_stats(self, mid_price):
        ...
        self.price_history_makalong.append(mid_price)
        if len(self.price_history_makalong) > self.PRICE_WINDOW:
            self.price_history_makalong.pop(0)

        if len(self.price_history_makalong) <= self.PRICE_WINDOW:
            self.mean_makalong = np.mean(self.price_history_makalong)
            self.sigma = np.std(self.price_history_makalong)
    ...
    def handle_basket_arbitrage1(self, state: TradingState):
        ...
        # 平仓逻辑（|Z|<0.5时平仓）
            ...
            if close_volume > 0:# 空头要平仓，买入
                best_bid = min(state.order_depths[basket_symbol].sell_orders.keys())
                orders.append(Order(basket_symbol, best_bid, close_volume))
            else:
                best_ask = max(state.order_depths[basket_symbol].buy_orders.keys())
            ...
            for p, qty in self.components.items():
                ...
                if hedge_volume > 0:# 空头平仓要买
                    best_bid = min(state.order_depths[p].buy_orders.keys())
                    orders.append(Order(p, best_bid, hedge_volume))
                else:
                    best_ask = max(state.order_depths[p].sell_orders.keys())
            ...
            for product in state.order_depths:
                if product in [Product.KELP, Product.SQUID_INK, Product.PICNIC_BASKET1,
                    ...
                    if product == Product.RAINFOREST_RESIN:
                        ...
                        orders = self.MRTraderObj.handle_KELP_trading(state)
                    elif product == Product.MAGNIFICENT_MACARONS:
                        orders, conversions = self.run_makalong(state)


                            # 火火火火火火火火火火火火火火火火火火火火火火火火火火火火火火火火火火火火火火火火火火火火火
                            ...
                            if price_limits:
                                ...
                                if best_bid and best_ask:
                                    if (
                                        ...
                                        buy_volume = min(
                                            ask_volume, self.MAX_POS - current_pos
                                        )
                                        if buy_volume > 0:
...
class MRTrader:
    ...
    def handle_KELP_trading(trader, state: TradingState):
        # 从state获取当前仓位和订单簿
        resin_position = state.position.get(Product.KELP, 0)
        order_depth = state.order_depths[Product.KELP]  # 直接取订单簿


        resin_take_orders, buy_order_volume, sell_order_volume = trader.take_orders(
            Product.KELP,
            state.order_depths[Product.KELP],
            trader.params[Product.KELP]["fair_value"],
            trader.params[Product.KELP]["take_width"],
            resin_position,
        )
        ...
        resin_clear_orders, buy_order_volume, sell_order_volume = trader.clear_orders(
            ...
            state.order_depths[Product.KELP],
            trader.params[Product.KELP]["fair_value"],
            trader.params[Product.KELP]["clear_width"],
            resin_position,
            buy_order_volume,
            sell_order_volume,
        )
        ...
        resin_make_orders, _, _ = trader.make_orders(
            ...
            buy_order_volume,
            sell_order_volume,
            trader.params[Product.KELP]["disregard_edge"],
            trader.params[Product.KELP]["join_edge"],
            trader.params[Product.KELP]["default_edge"],
            True,
            trader.params[Product.KELP]["soft_position_limit"],
        )
